...
# --- Copied from PFAPPO/train.py to avoid import issues ---
...
def compute_resource_weights(env, dwa_weights=None):
    ...
    STAR-PPO 特有：加入网络连通性感知（通过 link_latency 数据推断，不是硬编码陷阱ID）
    """
    ...
    # === STAR-PPO 核心：网络连通性感知 ===
    # 通过 link_latency 数据计算每个服务器的网络质量（公平：基于网络数据推断）
    ...
    if hasattr(env, 'link_latency') and len(env.link_latency) > 0:
        for i, sid in enumerate(server_ids):
            ...
            if outbound_lats:
                ...
                # 延迟越高，网络质量越差
                # 正常服务器: avg_lat ~ 100ms -> quality ~ 1.0
                # 差连接服务器: avg_lat ~ 2000ms -> quality ~ 0.05
...
def train(
    ...
    print(f"Starting STAR-PPO Training: {run_id}")
    print(f"Algorithm: State-Topology Augmented Resource-aware PPO")
    ...
    for epoch in range(total_epochs):
        ...
        # 超强版guidance：从0.6开始，快速增长到1.2
        guidance_alpha = 0.6 + 0.6 * progress  # 从0.6增长到1.2
