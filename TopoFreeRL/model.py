...
class StarActor(nn.Module):
    def __init__(self, state_dim, num_servers, hidden_dim=256):
        ...
        # Input: Augmented State (10) + Resource Weights (50)
        # STAR-PPO maintains the Resource-Awareness of PFAPPO but adds Topology awareness via State
        input_dim = state_dim + num_servers

        self.fc1 = nn.Linear(input_dim, hidden_dim)
        # Squeeze-and-Excitation Block for Feature Attention (Lightweight)
    ...
    def forward(self, state, resource_weights):
        # state: [batch, 10]
        # resource_weights: [batch, 50]

        x = torch.cat([state, resource_weights], dim=1)

        x = F.relu(self.fc1(x))

        # SE Block: Explicitly model feature importance
        # "Which features (Topology vs Base) matter now?"
        se = F.avg_pool1d(x.unsqueeze(1), kernel_size=1).squeeze(1) # Global pooling (identity for vector)
