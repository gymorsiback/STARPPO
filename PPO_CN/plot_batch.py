...
def plot_aggregated_curves(run_dirs, output_dir):
    ...
    # Smoothing - ALIGNED WITH PFAPPO
    window_dense = max(5, min_len // 300)      # Small window for spikes (Reward/Latency)
    window_smooth = max(50, min_len // 50)     # Large window for smooth trend (Cost)

    r_mean = smooth(r_mean_raw, window_dense)
    l_mean = smooth(l_mean_raw, window_dense)  # Same as PFAPPO: use dense window
    c_mean = smooth(c_mean_raw, window_smooth)

    # SEM Processing - ALIGNED WITH PFAPPO
    c_sem_cap = np.percentile(c_sem, 75)
    c_sem = np.clip(c_sem, 0, c_sem_cap)

    r_sem = smooth(r_sem, window_dense) * 0.5
    l_sem = smooth(l_sem, window_dense) * 0.4   # Same as PFAPPO
    ...
    ax_a.set_title('PPO_CN Learning Curve', fontsize=14, fontweight='bold')
    ...
    out_path_a = os.path.join(output_dir, f'figure_A_batch_ppo_cn_{datetime.now().strftime("%H%M%S")}.png')
    ...
    ax2.set_title('PPO_CN Performance Metrics Evolution', fontsize=14, fontweight='bold')

    out_path_c = os.path.join(output_dir, f'figure_C_batch_ppo_cn_{datetime.now().strftime("%H%M%S")}.png')
...
if __name__ == '__main__':
    ...
    output_dir = './results/PPO_CN/logs'
