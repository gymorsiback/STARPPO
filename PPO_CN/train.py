...
def build_server_model_mapping(ds, env):
    """
    构建 (server_index, model_type) -> list of model_instance_idx 的映射
    """
...
def map_server_action_to_instance(server_idx, required_model_type, mapping, ds, fallback_action=0):
    """
    将 Agent 选择的 Server ID 映射为具体的 Model Instance ID
    """
...
def build_state_vector(state_dict, dwa_weights):
    """
    构建 7维 状态向量 (与 PPO 一致)
    [StepNorm, TaskLon, TaskLat, RegionID, w_lat, w_cost, w_penalty]
    """
    return np.array([
...
def train(
    data_root='./data',
    total_epochs=100,      # Aligned with PPO
    episodes_per_epoch=200,
    lr=1e-4,               # Aligned with PPO (FIXED: was 3e-4)
    batch_size=1024,       # Aligned with PPO
    device='cpu',
    seed=42,
    noise_beta=1.0,  # 1.0 = Pink Noise (PPO_CN Special)
    noise_scale=0.03,      # Standard setting for colored noise exploration
    ...
    # Init Noise Explorer (PPO_CN Special)
    ...
    for epoch in range(total_epochs):
        ...
        # Noise Strategy: Colored Noise Exploration
        # 前30%训练: 使用彩色噪声增强探索 (线性衰减)
        # 后70%训练: 关闭噪声，纯策略学习
        # 这是 PPO_CN 的特色机制，不应人为削弱
        noise_cutoff = 0.30  # 30% epoch 后关闭

        if epoch < total_epochs * noise_cutoff:
            # 线性衰减到0
            progress = epoch / (total_epochs * noise_cutoff)
            current_scale = initial_scale * (1.0 - progress)  # 0.05 → 0
        else:
            current_scale = 0.0  # 完全关闭
        ...
        for ep in range(episodes_per_epoch):
            ...
            # GAE Calculation (aligned with PPO)
            returns = []
            gae = 0
            gamma = 0.99
            lam = 0.95
