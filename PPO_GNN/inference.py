...
def run_inference(
    ...
    # 跨域适配：只使用前MODEL_NUM_SERVERS个服务器
    ...
    # GNN uses candidate servers only
    server_ids = candidate_server_ids
    num_servers = len(server_ids)

    # Load Agent (must match train.py settings)
    ...
    # Build server to model mapping
    server_model_mapping = build_server_model_mapping(ds, env)

    # Edge index (K-NN graph, K=10)
    ...
    # Edge attributes (normalized distances)
    edge_attr_list = []
    for i, j in edge_indices:
        dist = np.linalg.norm(coords[i] - coords[j])
        edge_attr_list.append([dist / 20000.0])  # Normalize
    ...
    for i in range(min(episodes, len(ds.tasks))):
        ...
        # Get static features once per episode (only for candidate servers)
        # Custom implementation for cross-region adaptation
        ...
        while not done:
            # Build graph data - dynamic features for candidate servers only
            ...
            node_feats = torch.cat([static_feats, dynamic_feats], dim=1)  # [N, 3]

            global_feat = torch.FloatTensor(build_state_vector(state_dict, w)).unsqueeze(0).to(device)  # [1, 7]
            ...
            # Deterministic action (argmax) - time it
            import time
            t0 = time.time()
            with torch.no_grad():
                server_idx, _, _ = agent.act(graph_data, deterministic=True)
            ep_inference_time += (time.time() - t0) * 1000  # ms
