...
class PPOAgent:
    ...
    def act(self, graph_data, deterministic=False):
        """
        graph_data: single Data object or Batch object
        Returns:
            action_node_idx: [batch_size]
            log_prob: [batch_size]
            value: [batch_size]
        """
        self.net.eval()
    ...
    def update(self, memory, batch_size=64, gamma=0.99, gae_lambda=0.95, clip_param=0.2, ent_coef=0.01, update_iters=10):
        """
        memory: list of Data objects with attributes: action_node_idx, old_log_prob, ret, advantage
        """
        self.net.train()
        ...
        for _ in range(update_iters):
            for batch in loader:
                ...
                # ============================================
                # CRITICAL FIX: Ensure all tensors are 1D [B]
                # DataLoader may stack [1] tensors into [B, 1]
                # ============================================
