...
def plot_aggregated_curves(run_dirs, output_dir):
    ...
    # Smoothing
    window_dense = max(5, min_len // 300)      # Keep small for Reward
    window_latency = max(20, min_len // 100)   # Larger for Latency
    window_smooth = max(50, min_len // 50)     # Large for Cost

    r_mean = smooth(r_mean_raw, window_dense)
    l_mean = smooth(l_mean_raw, window_latency)
    ...
    # Latency SEM Exclusion
    l_sem_cap = np.percentile(l_sem, 75)
    l_sem = np.clip(l_sem, 0, l_sem_cap)

    r_sem = smooth(r_sem, window_dense) * 0.5
    l_sem = smooth(l_sem, window_latency) * 0.20  # Reduced width
    ...
    ax_a.set_title('PPO_GNN Learning Curve', fontsize=14, fontweight='bold')
    ...
    out_path_a = os.path.join(output_dir, f'figure_A_batch_ppo_gnn_{datetime.now().strftime("%H%M%S")}.png')
    fig_a.savefig(out_path_a, dpi=150, bbox_inches='tight')
    print(f"Saved Figure A: {out_path_a}")
    plt.close(fig_a)

    # Figure C: Latency & Cost
    ...
    ax1.set_title('PPO_GNN Performance Metrics Evolution', fontsize=14, fontweight='bold')

    out_path_c = os.path.join(output_dir, f'figure_C_batch_ppo_gnn_{datetime.now().strftime("%H%M%S")}.png')
...
if __name__ == '__main__':
    ...
    print(f"Found {len(latest_runs)} runs to plot:")
    ...
    output_dir = './results/PPO_GNN/logs'
