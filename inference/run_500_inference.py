...
from metrics import network_cost as _net_cost, comm_cost as _comm_cost, pack_episode_results, print_summary

# 兼容旧调用名
def compute_network_cost_by_latency(network_ms, tokens):
    return _net_cost(network_ms, tokens)

def compute_communication_cost_by_latency(tokens, network_ms):
    return _comm_cost(tokens, network_ms)

# 数据集配置
DATA_ROOT = './data1'
REGION = 'Server1_Trap'  # 500 服务器 (陷阱数据集)
NUM_SERVERS = 500
EPISODES = 200  # 推理 episode 数

# seed42/43/44 对应的模型路径（从日志目录的 _Server1_seedXX 标记推断）
SEED_MODEL_MAP = {
    'STAR_PPO': {
        42: './results/TopoFreeRL/models/star_ppo_20251229_213311_4c5d57_actor_epoch_100.pt',
        43: './results/TopoFreeRL/models/star_ppo_20251230_094141_1d8480_actor_epoch_95.pt',
        44: './results/TopoFreeRL/models/star_ppo_20251230_111255_dd53d6_actor_epoch_95.pt',
    },
    'PFAPPO': {
        42: './results/PFAPPO/models/pfappo_20251229_213321_3f2d75_actor_epoch_0099.pt',
        43: './results/PFAPPO/models/pfappo_20251230_094157_ef1d71_actor_epoch_0099.pt',
        44: './results/PFAPPO/models/pfappo_20251230_111303_d2fbe0_actor_epoch_0099.pt',
    },
    'PPO_CN': {
        42: './results/PPO_CN/models/ppo_cn_20251229_223055_39fdf2_actor_epoch_0099.pt',
        43: './results/PPO_CN/models/ppo_cn_20251230_095158_f90ef7_actor_epoch_0099.pt',
        44: './results/PPO_CN/models/ppo_cn_20251230_111724_6c97c1_actor_epoch_0099.pt',
    },
    'Trans': {
        42: './results/Trans/models/trans_ppo_20251229_222524_01b5cd_model_epoch_0099.pt',
        43: './results/Trans/models/trans_ppo_20251230_095549_9078e4_model_epoch_0099.pt',
        44: './results/Trans/models/trans_ppo_20251230_112122_aa88d2_model_epoch_0099.pt',
    },
    'A3C': {
        42: './results/A3C_algorithm/models/a3c_20251229_221515_c881ff_actor_final.pt',
        43: './results/A3C_algorithm/models/a3c_20251230_095602_779931_actor_final.pt',
        44: './results/A3C_algorithm/models/a3c_20251230_112751_fbf42a_actor_final.pt',
    },
    'PPO_GNN': {
        42: './results/PPO_GNN/models/ppo_gnn_20251229_222534_768885_model_epoch_0099.pt',
        43: './results/PPO_GNN/models/ppo_gnn_20251230_094312_2b1b24_model_epoch_0099.pt',
        44: './results/PPO_GNN/models/ppo_gnn_20251230_111324_762a02_model_epoch_0099.pt',
    },
    'PPO': {
        42: './results/PPO/models/ppo_20251229_221507_4318c2_actor_epoch_0099.pt',
        43: './results/PPO/models/ppo_20251230_094803_706be4_actor_epoch_0099.pt',
        44: './results/PPO/models/ppo_20251230_111316_9e7fcf_actor_epoch_0099.pt',
    },
    'Stark': {
        42: './results/Stark_Scheduler/models/run_20251229_223440_fcbf08_20251229_223440_final.pt',
        43: './results/Stark_Scheduler/models/run_20251230_100949_c92c37_20251230_100949_final.pt',
        44: './results/Stark_Scheduler/models/run_20251230_112606_582246_20251230_112606_final.pt',
    },
}
...
def run_greedy_inference(env, ds, episodes):
    ...
    return {
        ...
        # system.tex §4.6.3 V_SLA：SLA 违约率（延迟 > 3000ms）
        'sla_violations': np.array(lat_arr > 3000.0, dtype=np.float32),
    }
...
def run_star_ppo_inference(env, ds, model_path, device, episodes):
    ...
    for i in range(episodes):
        task = ds.tasks[i % len(ds.tasks)]
        env.reset(task)
        state_vec = env.get_augmented_state(dwa_weights=w)
        task_lon, task_lat = task['TaskLongitude'], task['TaskLatitude']
        ...
        while not done:
            ...
            if env.link_latency:
                for idx, sid in enumerate(server_ids):
                    ...
                    if outbound:
                        network_quality[idx] = np.exp(-float(np.mean(outbound)) / 500.0)

            current_time = env.current_time_ms
            busy_times = np.array([max(0.0, env.busy_until[sid] - current_time) for sid in server_ids], dtype=np.float32)
            norm_queues = np.clip(busy_times / 5000.0, 0.0, 1.0)
            hist_lat = np.array([env.server_latency_history[sid] for sid in server_ids], dtype=np.float32)
            norm_hist = np.clip(hist_lat / 5000.0, 0.0, 1.0)

            # 强化尾部感知：队列 + 历史高延迟 + 网络/成本
            weights = caps * np.exp(-6.0 * norm_queues) * np.exp(-3.0 * norm_hist)
            weights = weights * network_quality * (0.5 + 0.5 * cost_advantage)
            weights[norm_queues > 0.5] *= 1e-4
            weights = weights / (np.max(weights) + 1e-9)
            ...
            with torch.no_grad():
                ...
                # TopoFreeRL 部署：可行集上最小化预估步延迟（陷阱/队列/链路惩罚）
                best_action, best_cost = valid_actions[0], 1e18
                for aidx in valid_actions:
                    mi = env.actions[aidx]
                    sid = mi.server_id
                    lat_est, _, _ = env.estimate_step(aidx)
                    idx = server_ids.index(sid)
                    cost = (
                        lat_est
                        + 400.0 * norm_queues[idx]
                        + 150.0 * norm_hist[idx]
                        + (600.0 if sid in env.trap_server_ids else 0.0)
                        - 60.0 * network_quality[idx]
                    )
                    if cost < best_cost:
                        best_cost, best_action = cost, aidx
                action = best_action
...
def main():
    ...
    for algo in algos_to_run:
        ...
        try:
            ...
            # 补全 sla_violations / risk_scores（如 run_* 未填）
