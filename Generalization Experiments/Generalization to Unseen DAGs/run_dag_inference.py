#!/usr/bin/env python3
"""
Generalization to Unseen DAGs 实验
验证 STAR-PPO 在未见过的 DAG 拓扑上的泛化能力

拓扑类型：
1. Chain (链式): A -> B -> C -> D -> E (顺序依赖)
2. Diamond (菱形): A -> (B, C) -> D (分叉汇聚)
3. Tree (树状): A -> (B, C, D) (广播)
4. Hybrid (混合): 随机组合上述模式

对比算法：STAR-PPO, Greedy, Random
"""
...
REGION = 'Server1_Trap'
NUM_EPISODES = 100  # 每种拓扑测试的 episode 数
SEEDS = [42, 43, 44]
...
def run_star_ppo_inference(env: WorkflowMoEEnv, dag_env: DAGEnvironment,
    ...
    for task in tasks:
        ...
        for step_idx in range(num_steps):
            ...
            with torch.no_grad():
                logits = actor(state_tensor, resource_weights).squeeze(0)  # [num_servers]
            ...
            # 执行动作 - 在 DAG 环境中计算延迟/成本
            dag_env.step(action_idx, step_idx)

            # 关键修复：同时在 aug_env 中执行 step，以正确更新内部状态
            # (current_time_ms, busy_until, prev_server_id, step_idx)
            # 这确保 get_augmented_state 在下一步能获取正确的拓扑特征
            try:
                aug_env.step(action_idx)
            except:
                # 如果 aug_env.step 失败（例如 episode 结束），手动更新关键状态
...
def run_greedy_inference(env: WorkflowMoEEnv, dag_env: DAGEnvironment,
    ...
    """运行 Greedy 推理 - 限制信息量版本

    公平性说明：
    - 原版 Greedy 使用 env.estimate_step() 获取精确延迟估计（作弊）
    - 修改后只使用有限信息：距离 + 算力（与 STAR-PPO 公平）
    - 不知道链路延迟、陷阱信息等
    """
    np.random.seed(seed)
    random.seed(seed)
    ...
    for task in tasks:
        ...
        for step_idx in range(num_steps):
            ...
            for a_idx in available:
                ...
                # 简单启发式：距离 / 算力（距离越小越好，算力越大越好）
                # 注意：Greedy 不知道链路延迟、陷阱信息等
...
def main():
    ...
    # 计算各拓扑各算法的 SLA 违约率（system.tex §4.6.3，T_SLA=3000ms）
    sla_violations = {}
    for algo in ['STAR_PPO', 'Greedy', 'Random']:
        sla_violations[algo] = {}
        for topo in topologies:
            ...
            if valid:
                sla_violations[algo][topo.name] = float(np.mean(_sla(np.array(valid)))) * 100
            else:
                sla_violations[algo][topo.name] = 0.0
    ...
    np.savez(result_path,
             ...
             sla_viol_star=np.array([sla_violations['STAR_PPO'][t.name] for t in topologies]),
             sla_viol_greedy=np.array([sla_violations['Greedy'][t.name] for t in topologies]),
             sla_viol_random=np.array([sla_violations['Random'][t.name] for t in topologies]),
