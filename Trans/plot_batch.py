...
def plot_aggregated_curves(run_dirs, output_dir):
    ...
    # Smoothing
    window_dense = max(5, min_len // 300)      # Keep small for Reward/Latency
    ...
    ax_a.set_title('Trans-PPO Learning Curve', fontsize=14, fontweight='bold')
    ...
    out_path_a = os.path.join(output_dir, f'figure_A_batch_trans_{datetime.now().strftime("%H%M%S")}.png')
    ...
    # Fixed Y-axis for Latency (Aligned with others)
    ...
    ax2.set_title('Trans-PPO Performance Metrics Evolution', fontsize=14, fontweight='bold')

    out_path_c = os.path.join(output_dir, f'figure_C_batch_trans_{datetime.now().strftime("%H%M%S")}.png')
