...
def worker(rank, global_model, optimizer, global_ep_idx, global_ep_lock, res_queue,
    ...
    # Local Model
    # Important: If training on GPU, local_model is on GPU.
    # Global model is always on CPU (shared memory).
    ...
    while True:
        ...
        # Sync local model with global (CPU -> Device)
        # load_state_dict handles the copy automatically
        ...
        while not done:
            ...
            if done or len(rewards) >= update_freq:
                ...
                # Push gradients to global (Device -> CPU)
        ...
        # Episode finished - get latency/cost from env (like PPO does)
        ...
        res_queue.put({
            ...
            'ep_cost': ep_cost,
            'ep_switch': 0,  # Not tracked per-step in env
...
def train(data_root, total_epochs, episodes_per_epoch, seed, device_str='cuda', regions=None, output_dir=None):
    ...
    # Ensures results/A3C_algorithm structure
    ...
    while global_ep_idx.value < total_episodes or not res_queue.empty():
        try:
            ...
            if episodes_in_epoch >= episodes_per_epoch:
                ...
                # DWA Update Logic (Same as before)
