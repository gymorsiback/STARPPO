...
def plot_aggregated_curves(run_dirs, output_dir):
    ...
    n_runs = len(returns_matrix)
    r_sem = np.std(returns_matrix, axis=0) / np.sqrt(n_runs)
    l_sem = np.std(latency_matrix, axis=0) / np.sqrt(n_runs)
    c_sem = np.std(cost_matrix_filtered, axis=0) / np.sqrt(n_runs)

    # Smoothing windows - 与 PFAPPO 一致
    # Reward/Latency: 小窗口保持毛刺
    # Cost: 大窗口平滑，才能看出收敛趋势

    window_dense = max(5, min_len // 300)   # 小窗口，保持毛刺
    window_smooth = max(50, min_len // 50)  # 大窗口，Cost 专用

    r_mean = smooth(r_mean_raw, window_dense)
    l_mean = smooth(l_mean_raw, window_dense)
    c_mean = smooth(c_mean_raw, window_smooth)  # Cost 用大窗口平滑

    # Clip Cost SEM (与 PFAPPO 一致)
    ...
    c_sem = smooth(c_sem, window_smooth) * 0.10  # Cost 极窄误差带
    ...
    ax_a.set_title('PPO Learning Curve', fontsize=14, fontweight='bold')
    ...
    out_path_a = os.path.join(output_dir, f'figure_A_batch_ppo_{datetime.now().strftime("%H%M%S")}.png')
    fig_a.savefig(out_path_a, dpi=150, bbox_inches='tight')
    print(f"Saved Figure A: {out_path_a}")
    plt.close(fig_a)

    # ========================================
    # Figure C: Latency & Cost - 毛刺 Style
    # ========================================
    ...
    ax2.plot(x, c_mean, color=color_cost, linewidth=1.2)  # Cost 用粗线
    ...
    ax1.set_title('PPO Performance Metrics Evolution', fontsize=14, fontweight='bold')

    out_path_c = os.path.join(output_dir, f'figure_C_batch_ppo_{datetime.now().strftime("%H%M%S")}.png')
