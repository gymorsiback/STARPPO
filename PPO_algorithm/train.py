...
def build_server_model_mapping(ds, env):
    ...
    Same as in PFAPPO to ensure fair comparison.
    """
...
def build_state_vector(state_dict, dwa_weights):
    ...
    PPO 不使用 resource_weights，只有基础状态
    """
    return np.array([
...
def train(
    ...
    # Build Server -> Model Instance Mapping (保留 Action Space 映射修复)
    ...
    for epoch in range(total_epochs):
        ...
        for ep in range(episodes_per_epoch):
            task = random.choice(ds.tasks)  # 随机选择任务，防止 Agent 死记硬背
